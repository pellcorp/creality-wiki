...
The `#simple-af-support` channel has been setup for anyone wanting support for microprobe.
...
```
/usr/data/pellcorp/k1/installer.sh --install microprobe
```
...
Octoeverywhere can be installed onto the K1 when running the Microprobe.  However it is strongly recommend that you do not do this as the stress placed on the system by running octoeverywhere is significant.  The octoeverywhere companion running on another device on the same WIFI network as the K1 works great.
